let quoteEngine = [
 {
  quote: 'When the going gets tough, the tough get going.',
  quoter: 'Joe Kennedy'
 }, {
  quote: 'Opportunities don’t happen, you create them.',
  quoter: 'Chris Grosser'
 }, {
  quote: 'To accomplish great things, we must not only act, but also dream, not only plan, but also believe.',
  quoter: 'Antaloe France'
 }, {
  quote: 'Good things come to people who wait, but better things come to those who go out and get them.',
  quoter: 'Anonymous'
 }, {
  quote: 'We are what we repeatedly do. Excellence, then, is not an act, but a habit.',
  quoter: 'Aristotle'
 }, {
  quote: 'It is during our darkest moments that we must focus to see the light.',
  quoter: 'Aristotle Onassis'
 }, {
  quote: 'Always make a total effort, even when the odds are against you.',
  quoter: 'Arnold Palmer'
 }, {
  quote: 'Start where you are. Use what you have. Do what you can.',
  quoter: 'Arthur Ashe'
 }, {
  quote: 'Don’t wait for your feelings to change to take the action. Take the action and your feelings will change.',
  quoter: 'Barbara Baron'
 }, {
  quote: 'Action may not always bring happiness; but there is no happiness without action.',
  quoter: 'Benjamin Disraeli'
 }, {
  quote: 'The difference between a stumbling block and a stepping stone is how high you raise your foot.',
  quoter: 'Benny Lewis'
 }, {
  quote: 'As we look ahead into the next century, leaders will be those who empower others.',
  quoter: 'Bill Gates'
 }, {
  quote: 'Success comes from having dreams that are bigger than your fears.',
  quoter: 'Bobby Unser'
 }, {
  quote: 'If you want to lift yourself up, lift up someone else.',
  quoter: 'Booker T. Washington'
 }, {
  quote: 'You can never cross the ocean until you have the courage to lose sight of the shore.',
  quoter: 'Christopher Columbus'
 }, {
  quote: 'You measure the size of the accomplishment by the obstacles you had to overcome to reach your goals.',
  quoter: 'Booker T. Washington'
 }, {
  quote: 'The distance between insanity and genius is measured only by success.',
  quoter: 'Bruce Feirstein'
 }, {
  quote: 'Never do tomorrow what you can do today. Procrastination is the thief of time.',
  quoter: 'Charles Dickens'
 }, {
  quote: 'Don’t spend time beating on a wall, hoping to transform it into a door.',
  quoter: 'Coco Chanel'
 }, {
  quote: 'The road to success and the road to failure are almost exactly the same.',
  quoter: 'Colin R. Davis'
 }, {
  quote: 'The best way to predict the future is to create it.',
  quoter: 'Abraham Lincoln'
 }, {
  quote: 'There are far, far better things ahead than any we leave behind.',
  quoter: 'C.S. Lewis'
 }, {
  quote: 'Remember that not getting what you want is sometimes a wonderful stroke of luck.',
  quoter: 'Dalai Lama'
 }, {
  quote: 'Most of the important things in the world have been accomplished by people who have kept on trying when there seemed to be no help at all.',
  quoter: 'Dale Carnegie'
 }, {
  quote: 'A successful man is one who can lay a firm foundation with the bricks others have thrown at him.',
  quoter: 'David Brinkley'
 }, {
  quote: 'We become what we think about most of the time, and that’s the strangest secret.',
  quoter: 'Earl Nightingale'
 }, {
  quote: 'The reason most people never reach their goals is that they don’t define them, or ever seriously consider them as believable or achievable. Winners can tell you where they are going, what they plan to do along the way, and who will be sharing the adventure with them.',
  quoter: 'Denis Watiley'
 }, {
  quote: 'Develop success from failures. Discouragement and failure are two of the surest stepping stones to success.',
  quoter: 'Dale Carnegie'
 }, {
  quote: 'The question isn’t who is going to let me; it’s who is going.',
  quoter: 'Ayn Rand'
 }, {
  quote: 'Perfection is boring. Getting better is where all the fun is.',
  quoter: 'Dragos Roua'
 }, {
  quote: 'It\'s hard to beat a person who never gives up',
  quoter: 'Babe Ruth'
 }, {
  quote: 'There are two ways of spreading light. To be the candle, or the mirror that reflects it',
  quoter: 'Edith Wharton'
 }, {
  quote: 'The future belongs to those who believe in the beauty of their dreams',
  quoter: 'Eleanor Roosevelt'
 }, {
  quote: 'Our greatest fear should not be of failure but of succeeding at things in life that don’t really matter',
  quoter: 'Francis Chan'
 }, {
  quote: 'There is no chance, no destiny, no fate, that can hinder or control the firm resolve of a determined soul',
  quoter: 'Ella Wheeler Wilcox'
 }, {
  quote: 'Your focus determines your reality.',
  quoter: 'George Lucas'
 }, {
  quote: 'Build your own dreams, or someone else will hire you to build theirs',
  quoter: 'Farrah Gray'
 }, {
  quote: 'Do not wait; the time will never be \'just right.\' Start where you stand, and work with whatever tools you may have at your command, and better tools will be found as you go along',
  quoter: 'George Herbert'
 }, {
  quote: 'You\'ve got to get up every morning with determination if you’re going to go to bed with satisfaction',
  quoter: 'George Lorimer'
 }, {
  quote: 'Press forward. Do not stop, do not linger in your journey, but strive for the mark set before you',
  quoter: 'George Whitefield'
 }, {
  quote: 'Whether you think you can, or you can’t – either way you’re right',
  quoter: 'Henry Ford'
 }, {
  quote: 'If you genuinely want something, don’t wait for it — teach yourself to be impatient',
  quoter: 'Gurbaksh Chahal'
 }, {
  quote: 'Never give up, for that is just the place and time that the tide will turn',
  quoter: 'Harriet Beecher Stow'
 }, {
  quote: 'Never give in. Never, never, never, never—in nothing, great or small, large or petty—never give in, except to convictions of honour and good sense. Never yield to force. Never yield to the apparently overwhelming might of the enemy',
  quoter: 'Winston Churchill'
 }, {
  quote: 'It\'s not what you look at that matters, it\'s what you see',
  quoter: 'Henry David Thoreau'
 }, {
  quote: 'I can\'t change the direction of the wind, but I can adjust my sails to always reach my destination',
  quoter: 'Jimmy Dean'
 }, {
  quote: 'It is better to fail in originality than to succeed in imitation',
  quoter: 'Herman Melville'
 }, {
  quote: 'Limitations live only in our minds. But if we use our imaginations, our possibilities become limitless',
  quoter: 'Jamie Paolinetti'
 }, {
  quote: 'There is only one motivation, and that is desire. No reasons or principle contain it or stand against it.',
  quoter: 'Jane Smiley'
 }, {
  quote: 'The work you do when you procrastinate is probably the work you should be doing for the rest of your life',
  quoter: 'Jessica Hische'
 }, {
  quote: 'If you are not willing to risk the usual you will have to settle for the ordinary',
  quoter: 'Jim Rohn'
 }, {
  quote: 'Sometimes you win, sometimes you learn',
  quoter: 'John C. Maxwell'
 }, {
  quote: 'Challenges are what make life interesting. Overcoming them is what makes life meaningful',
  quoter: 'Joshua J. Marine'
 }, {
  quote: 'There are two kinds of people in this world; those who want to get things done, and those who don’t want to make mistakes.',
  quoter: 'John C. Maxwell'
 }, {
  quote: 'Don\'t be afraid to give up the good to go for the great',
  quoter: 'John D. Rockefeller'
 }, {
  quote: 'Don\'t let what you cannot do interfere with what you can do',
  quoter: 'John Wooden'
 }, {
  quote: 'Why should you continue going after your dreams\? Because seeing the look on the faces of the people who said you couldn\'t… will be priceless',
  quoter: 'Kevin Ngo'
 }, {
  quote: 'The harder the battle, the sweeter the victory',
  quoter: 'Les Brown'
 }, {
  quote: 'If you don\'t value your time, neither will others. Stop giving away your time and talents- start charging for it',
  quoter: 'Kim Garst'
 }, {
  quote: 'Make sure your worst enemy doesn\'t live between your own two ears',
  quoter: 'Laird Hamilton'
 }, {
  quote: 'I have been impressed with the urgency of doing. Knowing is not enough; we must apply. Being willing is not enough; we must do',
  quoter: 'Leonardo da Vinci'
 }, {
  quote: 'Life is not about finding yourself. Life is about creating yourself',
  quoter: 'Lolly D'
 }, {
  quote: 'Every now and then it’s good to stop climbing and appreciate the view from right where you are',
  quoter: 'Lori Deschene'
 }, {
  quote: 'All progress takes place outside the comfort zone',
  quoter: 'Michael John Bobak'
 }, {
  quote: 'Always remember that you are absolutely unique; just like everyone else.',
  quoter: 'Margaret Mead'
 }, {
  quote: 'You may have to fight a battle more than once to win it',
  quoter: 'Margaret Thatcher'
 }, {
  quote: 'If you have everything under control, you\'re not moving fast enough',
  quoter: 'Mario Andretti'
 }, {
  quote: 'Twenty years from now you will be more disappointed by the things that you didn\'t do than by the ones you did do, so throw off the bowlines, sail away from safe harbor, catch the trade winds in your sails. Explore, Dream, Discover',
  quoter: 'Mark Twain'
 }, {
  quote: 'The only person you should try to be better than, is the person you were yesterday',
  quoter: 'Matty Mullens'
 }, {
  quote: 'Nothing will work unless you do',
  quoter: 'Maya Angelou'
 }, {
  quote: 'You don\'t have to see the whole staircase, just take the first step',
  quoter: 'Martin Luther King Jr.'
 },
 {
  quote: 'I\'ve missed more than 9,000 shots in my career. I’ve lost almost 300 games. 26 times I’ve been trusted to take the game winning shot and missed. I\'ve failed over and over and over again in my life. And that is why I succeed',
  quoter: 'Michael Jordan'
},
 {
  quote: 'Obstacles don\'t have to stop you. If you run into a wall, don’t turn around and give up',
  quoter: 'Michael Jordan'
}, {
  quote: 'One way to keep momentum going is to have constantly greater goals',
  quoter: 'Michael Korda'
}, {
  quote: 'There will be obstacles. There will be doubters. There will be mistakes. But with hard work, there are no limits',
  quoter: 'Michael Phelps'
}, {
  quote: 'You can\'t put a limit on anything. The more you dream, the further you get',
  quoter: 'Michael Phelps'
}, {
  quote: 'To be successful you must accept all challenges that come your way. You can’t just accept the ones you like.',
  quoter: 'Mike Gafka'
}, {
  quote: 'It isn\'t the mountains ahead to climb that wear you out; it’s the pebble in your shoe',
  quoter: 'Muhammad Ali'
}, {
  quote: 'The starting point of all achievement is desire',
  quoter: 'Napoleon Hill'
}, {
  quote: 'Whatever the mind of man can conceive and believe, it can achieve',
  quoter: 'Napoleon Hill'
}, {
  quote: 'Believe in yourself\! Have faith in your abilities\! Without a humble but reasonable confidence in your own powers you cannot be successful or happy',
  quoter: 'Norman Vincent Peale'
}, {
  quote: 'You can give it all up here, lay low and \'die\'. Beyond is the homw of victors',
  quoter: 'Opeoluwa Adeoye'
}, {
  quote: 'The number one reason people fail in life is because they listen to their friends, family, and neighbors',
  quoter: 'Napoleon Hill'
}, {
  quote: 'Change your thoughts and you change your world',
  quoter: 'Norman Vincent Peale'
}, {
  quote: 'Find a victory in every defeat to remain hopeful, and find a defeat in every victory to remain humble',
  quoter: 'Orrin Woodward'
}, {
  quote: 'What seems to us as bitter trials are often blessings in disguise',
  quoter: 'Oscar Wilde'
}, {
  quote: 'Only put off until tomorrow what you are willing to die having left undone',
  quoter: 'Pablo Picasso'
}, {
  quote: 'There are only two options regarding commitment. You’re either in or you’re out. There is no such thing as life in-between',
  quoter: 'Pat Riley'
}, {
  quote: 'The tragedy in life doesn’t lie in not reaching your goal. The tragedy lies in having no goal to reach',
  quoter: 'Benjamin Mays'
}, {
  quote: 'More powerful than the will to win is the courage to begin',
  quoter: 'Orrin Woodward'
}, {
  quote: 'Where there is a will, there is a way. If there is a chance in a million that you can do something, anything, to keep what you want from ending, do it. Pry the door open or, if need be, wedge your foot in that door and keep it open',
  quoter: 'Pauline Kael'
}, {
  quote: 'When we strive to become better than we are, everything around us becomes better too',
  quoter: 'Paulo Coelho'
}, {
  quote: 'You may only succeed if you desire succeeding; you may only fail if you do not mind failing',
  quoter: 'Philippos'
}, {
  quote: 'What we achieve inwardly will change outer reality',
  quoter: 'Plutarch'
}, {
  quote: 'The difference between the impossible and the possible lies in a person’s determination',
  quoter: 'Tommy Lasorda'
}, {
  quote: 'Just when the caterpillar thought the world was ending, he turned into a butterfly',
  quoter: 'Proverb'
}, {
  quote: 'We aim above the mark to hit the mark',
  quoter: 'Ralph Waldo Emerson'
}, {
  quote: 'Don’t be pushed by your problems; be led by your dreams',
  quoter: 'Ralph Waldo Emerson'
}, {
  quote: 'There are two types of people who will tell you that you cannot make a difference in this world: those who are afraid to try and those who are afraid you will succeed',
  quoter: 'Ray Goforth'
}, {
  quote: 'You don’t drown by falling in water; you drown by staying there.',
  quoter: 'Robert Collier'
}, {
  quote: 'Don\'t watch the clock; do what it does. Keep going',
  quoter: 'Sam Levenson'
}, {
  quote: 'Don\'t let the fear of losing be greater than the excitement of winning.',
  quoter: 'Robert Kiyosaki'
}, {
  quote: 'Don\'t judge each day by the harvest you reap but by the seeds that you plant',
  quoter: 'Robert Louis Stevenson'
}, {
  quote: 'Better to do something imperfectly than to do nothing flawlessly',
  quoter: 'Robert Schuller'
}, {
  quote: 'Don\'t live the same year 75 times and call it a life',
  quoter: 'Robin Sharma'
}, {
  quote: 'The more difficult the victory, the greater the happiness in winning',
  quoter: 'Pele'
}, {
  quote: 'We can\'t help everyone, but everyone can help someone',
  quoter: 'Ronald Reagan'
}, {
  quote: 'If you\'re offered a seat on a rocket ship, don\'t ask what seat\! Just get on',
  quoter: 'Sheryl Sandberg'
}, {
  quote: 'Change your life today. Don\'t gamble on the future, act now, without delay',
  quoter: 'Simone de Beauvoir'
}, {
  quote: 'I am not a product of my circumstances. I am a product of my decisions',
  quoter: 'Stephen Covey'
}, {
  quote: 'Motivation is a fire from within. If someone else tries to light that fire under you, chances are it will burn very briefly',
  quoter: 'Stephen R Covey'
}, {
  quote: 'Don\'t downgrade your dream just to fit your reality. Upgrade your conviction to match your destiny',
  quoter: 'Stuart Scott'
}, {
  quote: 'When everything\'s coming your way, you\'re in the wrong lane',
  quoter: 'Steven Wright'
}, {
  quote: 'If you want to make a permanent change, stop focusing on the size of your problems and start focusing on the size of you\!',
  quoter: 'T. Harv Eker'
}, {
  quote: 'When you\'ve got something to prove, there\'s nothing greater than a challenge.',
  quoter: 'Terry Bradshaw'
}, {
  quote: 'Keep your eyes on the stars, and your feet on the ground',
  quoter: 'Theodore Roosevelt'
}, {
  quote: 'Our greatest weakness lies in giving up. The most certain way to succeed is always to try just one more time',
  quoter: 'Thomas Edison'
}, {
  quote: 'Zeal without knowledge is fire without light',
  quoter: 'Thomas Huxley'
}, {
  quote: 'Believe you can and you’re halfway there',
  quoter: 'Theodore Roosevelt'
}, {
  quote: 'If you want to achieve excellence, you can get there today. As of this second, quit doing less-than-excellent work',
  quoter: 'Thomas J. Watson'
}, {
  quote: 'I find that the harder I work, the more luck I seem to have.',
  quoter: 'Thomas Jefferson'
}, {
  quote: 'If you want something you\'ve never had, you must be willing to do something you\'ve never done',
  quoter: 'Thomas Jefferson'
}, {
  quote: 'The individual who says it is not possible should get out of the way of those doing it',
  quoter: 'Tricia Cunningham'
}, {
  quote: 'Failure is the condiment that gives success its flavor',
  quoter: 'Truman Capote'
}, {
  quote: 'Perfection is not attainable, but if we chase perfection we can catch excellence',
  quoter: 'Vince Lombardi'
}, {
  quote: 'Throw me to the wolves and I will return leading the pack',
  quoter: 'Unknown'
}, {
  quote: 'There are better starters than me, but I’m a strong finisher',
  quoter: 'Usain Bolt'
}, {
  quote: 'The only place where success comes before work is in the dictionary.',
  quoter: 'Vidal Sassoon'
}, {
  quote: 'I would rather die of passion than of boredom',
  quoter: 'Vincent van Gogh'
}, {
  quote: 'Fortune sides with him who dares',
  quoter: 'Virgil'
}, {
  quote: 'Aim for the moon. If you miss, you may hit a star',
  quoter: 'W. Clement Stone'
}, {
  quote: 'If you can dream it, you can do it.',
  quoter: 'Walt Disney'
}, {
  quote: 'You must not let anyone define your limits because of where you come from. Your only limit is your soul',
  quoter: 'Walt Disney'
}, {
  quote: 'Little minds are tamed and subdued by misfortune; but great minds rise above it',
  quoter: 'Washington Irving'
}, {
  quote: 'Be miserable. Or motivate yourself. Whatever has to be done, it’s always your choice',
  quoter: 'Wayne Dyer'
}, {
  quote: 'All our dreams can come true if we have the courage to pursue them',
  quoter: 'Walt Disney'
}, {
  quote: 'Do not wait to strike till the iron is hot; but make it hot by striking',
  quoter: 'William Butler Yeats'
}, {
  quote: 'Do or do not. There is no try',
  quoter: 'Yoda'
}, {
  quote: 'When one door of happiness closes, another opens, but often we look so long at the closed door that we do not see the one that has been opened for us.',
  quoter: 'Helen Keller'
}, {
  quote: 'People often say that motivation doesn’t last. Well, neither does bathing – that’s why we recommend it daily',
  quoter: 'Zig Ziglar'
}, {
  quote: 'Strive not to be a success, but rather to be of value',
  quoter: 'Albert Einstein'
}, {
  quote: 'The most difficult thing is the decision to act, the rest is merely tenacity',
  quoter: 'Amelia Earhart'
}, {
  quote: 'The most common way people give up their power is by thinking they don’t have any',
  quoter: 'Alice Walker'
}, {
  quote: 'Certain things catch your eye, but pursue only those that capture the heart.',
  quoter: 'Ancient Indian Proverb'
}, {
  quote: 'How wonderful it is that nobody need wait a single moment before starting to improve the world',
  quoter: 'Anne Frank'
}, {
  quote: 'Real difficulties can be overcome; it is only the imaginary ones that are unconquerable',
  quoter: 'Theodore N. Vail'
}, {
  quote: 'A diamond is merely a lump of coal that did well under pressure.',
  quoter: 'Unknown'
}, {
  quote: 'A successful man is one who can lay a firm foundation with the bricks others have thrown at him.',
  quoter: 'David Brinkley'
}, {
  quote: 'All men who have achieved great things have been great dreamers',
  quoter: 'Orison Swett Marden'
}, {
  quote: 'All of the great leaders have had one characteristic in common: it was the willingness to confront unequivocally the major anxiety of their people in their time. This, and not much else, is the essence of leadership',
  quoter: 'John Kenneth Galbraith'
}, {
  quote: 'An invincible determination can accomplish almost anything and in this lies the great distinction between great men and little men',
  quoter: 'Thomas Fuller'
}, {
  quote: 'The best way out is always through',
  quoter: 'Robert Frost'
}, {
  quote: 'By recording your dreams and goals on paper, you set in motion the process of becoming the person you most want to be. Put your future in good hands — your own',
  quoter: 'Mark Victor Hansen'
}, {
  quote: 'Desire is the key to motivation, but it’s determination and commitment to an unrelenting pursuit of your goal — a commitment to excellence — that will enable you to attain the success you seek',
  quoter: 'Mario Andretti'
}, {
  quote: 'Don\'t worry about the world coming to an end today. It\'s already tomorrow in Australia',
  quoter: 'Charles Schulz'
}, {
  quote: 'Dream as if you’ll live forever, live as if you’ll die today',
  quoter: 'James Dean'
}, {
  quote: 'Enthusiasm moves the world',
  quoter: 'Arthur Balfour'
}, {
  quote: 'Enter every activity without giving mental recognition to the possibility of defeat. Concentrate on your strengths, instead of your weaknesses… on your powers, instead of your problems',
  quoter: 'Paul J. Meyer'
}, {
  quote: 'Everything that irritates us about others can lead us to an understanding of ourselves',
  quoter: 'Carl Jung'
}, {
  quote: 'Give yourself an even greater challenge than the one you are trying to master and you will develop the powers necessary to overcome the original difficulty',
  quoter: 'William J. Bennett, The Book of Virtues'
}, {
  quote: 'A person who never made a mistake never tried anything new',
  quoter: 'Albert Einstein'
}, {
  quote: 'I know what I have to do, and I’m going to do whatever it takes. If I do it, I’ll come out a winner, and it doesn’t matter what anyone else does',
  quoter: 'Florence Griffith Joyner'
}, {
  quote: 'Winners never quit, and quitters never win',
  quoter: 'Anonymous'
}, {
  quote: 'If you are going to achieve excellence in big things, you develop the habit in little matters. Excellence is not an exception, it is a prevailing attitude.',
  quoter: 'Charles R. Swindoll'
}, {
  quote: 'Focus on the journey, not the destination. Joy is found not in finishing an activity but in doing it',
  quoter: 'Greg Anderson'
}, {
  quote: 'If you start to think the problem is ‘out there,’ stop yourself. That thought is the problem',
  quoter: 'Stephen Covey'
}, {
  quote: 'In a gentle way, you can shake the world.',
  quoter: 'Mahatma Gandhi'
}, {
  quote: 'It is a paradoxical but profoundly true and important principle of life that the most likely way to reach a goal is to be aiming not at that goal itself but at some more ambitious goal beyond it',
  quoter: 'Arnold Toynbee'
}, {
  quote: 'Nurture your mind with great thoughts. To believe in the heroic makes heroes',
  quoter: 'Benjamin Disraeli'
}, {
  quote: 'Knowing trees, I understand the meaning of patience. Knowing grass, I can appreciate persistence',
  quoter: 'Hal Borland'
},
 {
  quote: 'Yesterday is not ours to recover, but tomorrow is ours to win or lose',
  quoter: 'Lyndon Johnson'

},
 {
  quote: 'When nothing seems to help, I go and look at a stonecutter hammering away at his rock perhaps a hundred times without as much as a crack showing in it. Yet at the hundred and first blow it will split in two, and I know it was not that blow that did it – but all that had gone before',
  quoter: 'Jacob Riis'

 }, {
  quote: 'Do what you have to do until you can do what you want to do',
  quoter: 'Oprah Winfrey'

 }, {
  quote: 'Would you like me to give you a formula for success? It’s quite simple, really. Double your rate of failure. You are thinking of failure as the enemy of success. But it isn’t at all. You can be discouraged by failure or you can learn from it, So go ahead and make mistakes. Make all you can. Because remember that’s where you will find success',
  quoter: 'Thomas J. Watson'

}, {
  quote: 'Your present circumstances don’t determine where you can go; they merely determine where you start',
  quoter: 'Nido Qubein'

}, {
  quote: 'What great thing would you attempt if you knew you could not fail\?',
  quoter: 'Robert H. Schuller'

 }, {
  quote: 'We must train from the inside out. Using our strengths to attack and nullify any weaknesses. It’s not about denying a weakness may exist but about denying its right to persist',
  quoter: 'Vince McConnell'

 }, {
  quote: 'The task of leadership is not to put greatness into people, but to elicit it, for the greatness is there already',
  quoter: 'John Buchan'
}, {
  quote: 'With self-discipline most anything is possible',
  quoter: 'Theodore Roosevelt'

}, {
  quote: 'The secret of joy in work is contained in one word – excellence. To know how to do something well is to enjoy it',
  quoter: 'Pearl Buck'

 }, {
  quote: 'If you’re going through hell, keep going',
  quoter: 'Winston Churchill'

 }, {
  quote: 'There is no agony like bearing an untold story inside of you',
  quoter: 'Maya Angelou'

}, {
  quote: 'Successful and unsuccessful people do not vary greatly in their abilities. They vary in their desires to reach their potential',
  quoter: 'John Maxwell'

}, {
  quote: 'Everything you’ve ever wanted is on the other side of fear',
  quoter: 'George Addair'

 }, {
  quote: 'Success is not the key to happiness. Happiness is the key to success. If you love what you are doing, you will be successful',
  quoter: 'Albert Schweitzer'

 }, {
  quote: 'Much of the stress that people feel doesn\'t come from having too much to do. It comes from not finishing what they\'ve started',
  quoter: 'David Allen'

}, {
  quote: 'Study while others are sleeping; work while others are loafing; prepare while others are playing; and dream while others are wishing',
  quoter: 'William Arthur Ward'

}, {
  quote: 'Nothing is impossible, the word itself says \'I\'m possible\'\!',
  quoter: 'Audrey Hepburn'

 }, {
  quote: 'Put your heart, mind, and soul into even your smallest acts. This is the secret of success',
  quoter: 'Lance Armstrong'
 },
 {
  quote: 'Nothing in the world can take the place of Persistence. Talent will not; nothing is more common than unsuccessful men with talent. Genius will not; unrewarded genius is almost a proverb. Education will not; the world is full of educated derelicts. Persistence and determination alone are omnipotent. The slogan ‘Press On’ has solved and always will solve the problems of the human race.',
  quoter: 'Calvin Coolidge'
 },
 {
  quote: 'Nobody can go back and start a new beginning, but anyone can start today and make a new ending',
  quoter: 'Maria Robinson'
  }, {
  quote: 'My attitude is that if you push me towards something that you think is a weakness, then I will turn that perceived weakness into a strength',
  quoter: 'Michael Jordan'
   },
 {
  quote: 'Live with intention. Walk to the edge. Listen hard. Practice wellness. Play with abandon. Laugh. Choose with no regret. Do what you love. Live as if this is all there is',
  quoter: 'Mary Anne Roadacher-Hershey'
   }
];
// initialize
function randomQuoteEngine(nodeId) {
 //ref start and end point of quote engine object 
 let max = quoteEngine.length + 1;
 let min = 1;
 //avoid DRY: commit a value to a holder index such that the quote and quoter index in the root array matches
 let index = Math.round((max - min) * Math.random() + min);
 //return quote and quoter
 let quoteArr = [quoteEngine[index].quote, quoteEngine[index].quoter];
 //ref wrapper
 let container = document.getElementById(`${nodeId}`);
 //dynamically create quote node 
 let quoteContainer = document.createElement('div');
 //dynamically create quoter node
 let quoterContainer = document.createElement('small');
 //define quoteContainer and quoteContainer style 
 quoteContainer.style.marginBottom = '1rem';
 quoteContainer.style.fontSize = '102%';
 quoteContainer.style.fontWeight = '400';
 quoteContainer.style.textAlign = 'center';
 quoterContainer.style.textAlign = 'center';
 quoterContainer.style.display = 'block';
 quoterContainer.style.fontSize = '80 %';
 quoterContainer.style.color = '#6c757d';
 //dynamically create quote node content
 let quote = document.createTextNode(quoteArr[0]);
 //dynamically create quoter node content
 let quoter = document.createTextNode(quoteArr[1]);
 //append text node to wrapper{quote and quoter}
 quoteContainer.appendChild(quote);
 quoterContainer.appendChild(quoter);
 //append quoteContainer and quoterContainer to wrapper
 container.appendChild(quoterContainer);
 container.insertBefore(quoteContainer, quoterContainer);
}